...
public function connect_compte($u, $p)
{
    ...
    if ($resultat->getNumRows() > 0) {
        ...
        // Si invité : statut = Membre
        if ($ligne->pfl_statut === null) {
            $ligne->pfl_statut = "Membre";
        }
...
// récupère toutes les ressources (id + nom)
public function get_all_resources()
{
    $res = $this->db->query("SELECT rsc_id, rsc_nom FROM t_ressource_rsc ORDER BY rsc_nom ASC;");
    return $res->getResultArray();
}

// liste des ressources
public function get_all_resources_full()
{
    $sql = "SELECT rsc_id, rsc_nom, rsc_photo, rsc_jauge 
...
//inserer une ressource 
public function insert_ressource($nom, $photo, $description, $jauge, $url)
{
    $sql = "INSERT INTO t_ressource_rsc (rsc_nom, rsc_photo, rsc_description, rsc_jauge, rsc_url)
            VALUES (?, ?, ?, ?, ?)";

    return $this->db->query($sql, [$nom, $photo, $description, $jauge, $url]);
}



// supprimer une ressource 
public function delete_ressource_cascade($rsc_id)
{
    // Récupérer toutes les réservations de la ressource
    $res = $this->db->query("SELECT res_id FROM t_reservation_res WHERE rsc_id=?", [$rsc_id])
                    ->getResultArray();
    ...
    foreach ($res as $r) {
        ...
        $this->db->query("DELETE FROM t_inscription_isc WHERE res_id=?", [$res_id]);

        // supprimer réservation
        $this->db->query("DELETE FROM t_reservation_res WHERE res_id=?", [$res_id]);
    }

    // supprimer états
    $this->db->query("DELETE FROM t_etat_ett WHERE rsc_id=?", [$rsc_id]);

    // supprimer ressource
    $this->db->query("DELETE FROM t_ressource_rsc WHERE rsc_id=?", [$rsc_id]);
}
...
public function get_reservations_by_day($date)
{
    $sql = "
        SELECT 
            ...
            (SELECT c.cpt_pseudo
             FROM t_inscription_isc i
             JOIN t_compte_cpt c ON c.cpt_id = i.cpt_id
             WHERE i.res_id = res.res_id AND i.isc_role = 'Conducteur'
             LIMIT 1) AS responsable,

            -- Participants
            (SELECT GROUP_CONCAT(c2.cpt_pseudo SEPARATOR ', ')
             FROM t_inscription_isc i2
             JOIN t_compte_cpt c2 ON c2.cpt_id = i2.cpt_id
             WHERE i2.res_id = res.res_id AND i2.isc_role = 'Passager'
            ) AS participants

        FROM t_ressource_rsc rsc
        LEFT JOIN t_reservation_res res 
            ON rsc.rsc_id = res.rsc_id
            AND DATE(res.res_date_debut) = ?

        ORDER BY rsc.rsc_nom, res.res_date_debut
    ";
